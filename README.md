...
- «Задержка склада (сводная)» (агрегация всех Excel из отдельной папки Я.Диска в отдельный worksheet)
...
- Постоянные кнопки:
  ...
  - 📦 Задержка склада (сводная) — чтение всех Excel/zip из `YANDEX_WAREHOUSE_DELAY_DIR`, построение двух блоков и полная перезапись отдельного worksheet.
...
- Отдельный worksheet для «Задержка склада (сводная)»:
  ...
  - Содержит 2 блока: общий и `без задания`.
...
2. Выберите режим:
   ...
   - 📦 Задержка склада (сводная) — ничего отправлять не нужно; бот сам скачает все файлы из `YANDEX_WAREHOUSE_DELAY_DIR` и обновит отдельный worksheet.
...
- При нажатии «📦 Задержка склада (сводная)» бот читает все подходящие файлы из `YANDEX_WAREHOUSE_DELAY_DIR`, скачивает их по очереди во временную директорию и после обработки удаляет временные файлы.
